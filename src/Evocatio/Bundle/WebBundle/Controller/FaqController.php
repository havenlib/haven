...
class FaqController extends ContainerAware {
    ...
    public function indexAction() {
        $entities = $this->container->get("evocatio_web.faq.read_handler")->getAllPublished();
    ...
    public function rankAction($id) {
        $entity = $this->container->get("evocatio_web.faq.read_handler")->get($id);
        $form_data = $this->container->get("request")->get('form');
        $new_rank = (int) $form_data['rank'];

        if (is_int($new_rank) && $new_rank) {
            $this->container->get("evocatio_web.faq.persistence_handler")->rank($entity, $new_rank);
    ...
    public function listAction() {
        $entities = $this->container->get("evocatio_web.faq.read_handler")->getAll();


        foreach ($entities as $entity) {
            $delete_forms[$entity->getId()] = $this->container->get("evocatio_web.faq.form_handler")->createDeleteForm($entity->getId())->createView();
            $rank_forms[$entity->getId()] = $this->container->get("evocatio_web.faq.form_handler")->createRankForm($entity->getId(), $entity->getRank())->createView();
        }
    ...
    public function createAction() {
        $edit_form = $this->container->get("evocatio_web.faq.form_handler")->createNewForm();
    ...
    public function editAction($id) {
        $entity = $this->container->get('evocatio_web.faq.read_handler')->get($id);
        $edit_form = $this->container->get("evocatio_web.faq.form_handler")->createEditForm($entity->getId());
        $delete_form = $this->container->get("evocatio_web.faq.form_handler")->createDeleteForm($entity->getId());
    ...
    public function updateAction($id) {
        ...
        $edit_form->bind($this->container->get('Request'));
        if ($edit_form->isValid()) {
            $this->container->get("evocatio_web.faq.persistence_handler")->save($edit_form->getData());
            $this->container->get("session")->getFlashBag()->add("success", "create.success");
    ...
    public function deleteAction() {
        ...
        $form_data = $this->container->get("request")->get('form');
        $this->container->get("evocatio_web.faq.persistence_handler")->delete($form_data['id']);
