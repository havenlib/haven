...
//// Evocatio includes
//use Evocatio\Bundle\WebBundle\Entity\ProductTranslation as EntityTranslation;
...
class ProductController extends ContainerAware {
    ...
    public function indexAction() {
        $entities = $this->container->get("evocatio_pos.product.read_handler")->getAllPublished();
    ...
    public function rankAction($id) {
        $entity = $this->container->get("evocatio_pos.product.read_handler")->get($id);
        $form_data = $this->container->get("request")->get('form');
        $new_rank = (int) $form_data['rank'];

        if (is_int($new_rank) && $new_rank) {
            $this->container->get("evocatio_pos.product.persistence_handler")->rank($entity, $new_rank);
    ...
    public function showAction($id) {
        $entity = $this->container->get("evocatio_pos.product.read_handler")->get($id);
        $delete_form = $this->container->get("evocatio_pos.product.form_handler")->createDeleteForm($id);
    ...
    public function listAction() {
        $entities = $this->container->get("evocatio_pos.product.read_handler")->getAll();
        foreach ($entities as $entity) {
            $delete_forms[$entity->getId()] = $this->container->get("evocatio_pos.product.form_handler")->createDeleteForm($entity->getId())->createView();
        }
    ...
    public function displayAction($slug) {
        ...
        $entity = $this->container->get("evocatio_pos.product.read_handler")->getByLocalizedSlug($slug, $locale);
    ...
    public function createAction() {
        $edit_form = $this->container->get("evocatio_pos.product.form_handler")->createNewForm();
        return array(
    ...
    public function addAction() {
        ...
        if ($edit_form->isValid()) {
            $this->container->get("evocatio_pos.product.persistence_handler")->save($edit_form->getData());
    ...
    public function editAction($id) {
        $entity = $this->container->get('evocatio_pos.product.read_handler')->get($id);
        $edit_form = $this->container->get("evocatio_pos.product.form_handler")->createEditForm($entity->getId());
        $delete_form = $this->container->get("evocatio_pos.product.form_handler")->createDeleteForm($entity->getId());
    ...
    public function updateAction($id) {
        ...
        if ($edit_form->get('save')->isClicked() && $edit_form->isValid()) {
            ...
            if ($edit_form->get('template')->isClicked()) {
                $edit_form = $this->container->get("evocatio_pos.product.form_handler")->createNewForm($edit_form->getData());
            } else {
                $this->container->get("session")->getFlashBag()->add("error", "create.error");
            }
        }
     ...
     *
     * @Route("/product/{id}/state", name="EvocatioPosBundle_ProductToggleState")
     * @Method("GET")
     */
    ...
    public function deleteAction() {
        ...
        $form_data = $this->container->get("request")->get('form');
        $this->container->get("evocatio_pos.product.persistence_handler")->delete($form_data['id']);
...
//    }
    ...
    public function listWidgetAction($template = null, $maximum = null) {
        $repo = $this->container->get('doctrine')->getRepository("EvocatioPosBundle:Product");
        $entities = $repo->findLastCreatedOnline($maximum);


        return new Response($this->container->get('templating')->render($template ? $template : 'EvocatioPosBundle:Product:list_widget.html.twig', array('entities' => $entities)));
    }
