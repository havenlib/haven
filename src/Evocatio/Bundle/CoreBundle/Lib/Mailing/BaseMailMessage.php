...
//            la boucle cherche pour les namespace dans les mails et done la valeur voulu à return value si toujours pas de valeur après assign default
    ...
    public function getNom(){
        $destinataire_data =  $this->getDestinataireData();
        
        if(key_exists("nom", $destinataire_data))
            return $destinataire_data["nom"];
        
        return $this->getCurrentTo();
    }
     ...
     *
     * exemple 1
     *       $notify = new PasswordResetMailMessage();
        $notify->setBaseHtmlBody(get_partial('global/emails/htmlTemplate', array('template_params' => $template_params)));
        $notify->setBaseTextBody(get_partial('global/emails/textTemplate', array('template_params' => $template_params)));
        $destinataire[sfConfig::get('app_email_demandeajoutintervenant')] = array();
        $notify->setBaseSujet($subject . " - " . sfConfig::get("app_site_name"));
        $notify->setDestinataire($destinataire);
        MailService::sendCurrentMessage($notify, sfConfig::get('app_dev_email_activation'));
exemple 2
        $insides = get_partial($this->_template_name);
        $template_params = array("content" => $insides, "is_partial" => false);
        $html_template = get_partial('global/emails/htmlTemplate', array('template_params' => $template_params));
        $text_template = get_partial('global/emails/textTemplate', array('template_params' => $template_params));

        $activation = $this->_user->getUserActivation()->getFirst();

        $notify = new PasswordResetMailMessage();
        $notify->setBaseHtmlBody("$html_template");
        $notify->setBaseTextBody($text_template);
        $destinataire[$this->_user->getPrincipalMail()] = array('/%firstname%/' => $this->_user->getFirstName(),
            '/%lastname%/' => $this->_user->getLastName(),
            '/%password%/' => $this->_pass,
            '/%url%/' => $notify->getBaseUrl() . 'activation?uuid=' . $activation->getUuid()
        );
        $notify->setBaseSujet($this->_email_subject . ' - ' . "%firstname% %lastname% - " . sfConfig::get("app_site_name"));
        $notify->setDestinataire($destinataire);
        MailService::sendCurrentMessage($notify, sfConfig::get('app_dev_email_activation'));
exemple 3
            $notify = new PasswordResetMailMessage();
            $notify->setBaseHtmlBody($this->getPartial('fdagalerie/patterns/body_email', array("store_user" => $store_user, "new_password" => $new_password)));
            $destinataire[$store_user->getMail()] = array();
            $notify->setBaseSujet("Information sur votre compte de la galerie de la FDA");
            $notify->setDestinataire($destinataire);
            MailService::sendCurrentMessage($notify, sfConfig::get('app_dev_email_activation'));
exemple 4
                $notify = new PasswordResetMailMessage();
                $notify->setFrom(array("/default/" => $this->getUser()->getEffectiveUser()->getEmailAddress()  ,
                     "/uda.ca/" => sfConfig::get('app_nonuda_from_email', 'noreply@agencered.ca')));
                $notify->setBaseHtmlBody($this->getPartial('global/emails/htmlTemplate', array('template_params' => $template_params)));
                $notify->setBaseTextBody($this->getPartial('global/emails/textTemplate', array('template_params' => $template_params)));
                $destinataire[$this->form->getValue("email")] = array();
                $notify->setBaseSujet($this->form->getValue("subject") . " - " . sfConfig::get("app_site_name"));
                $notify->setDestinataire($destinataire);
                MailService::sendCurrentMessage($notify, sfConfig::get('app_dev_email_activation'));
     */
}
